...
from keypoints_to_graph_hard_pos import *
import pickle
...
def test_load_data_for_gcn_train():
    ...
    output_folder = "."
    data_out_path = '{}/posetrack_train_data_hard_pos.pickle'.format(output_folder)
    with open(data_out_path, 'wb') as handle:
        pickle.dump(graph_pair_list_all, handle)

    with open('./posetrack_train_data_hard_pos.pickle', 'rb') as handle:
...
def test_load_data_for_gcn_val():
    ...
    output_folder = "."
    data_out_path = '{}/posetrack_val_data_hard_pos.pickle'.format(output_folder)
    with open(data_out_path, 'wb') as handle:
        pickle.dump(graph_pair_list_all, handle)

    with open('./posetrack_val_data_hard_pos.pickle', 'rb') as handle:
        restore = pickle.load(handle)
    print(restore == graph_pair_list_all)
